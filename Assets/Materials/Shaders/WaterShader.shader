...
#pragma surface surf Standard vertex:vert addshadow alpha alphatest:_Cutoff
...
#pragma glsl
	...
	//The wave function
	float3 getWavePos(float3 pos, float norm)
	{
		float waveType = pos.z;

		pos *= (sin((_WaterTime * _WaterSpeed + waveType) / _WaterDistance) * _WaterScale); //* tex2Dlod(_NoiseTex, float4(pos.x, pos.z + sin(_WaterTime * 0.1), 0.0, 0.0) * _WaterNoiseWalk).a * _WaterNoiseStrength)/10;
	...
	void vert(inout appdata_full IN)
	{
		...
		float3 withWave = getWavePos(worldPos.xyz, IN.normal);

		//Convert the position back to local
		float4 localPos = mul(unity_WorldToObject, float4(worldPos.xyz + withWave, worldPos.w));
